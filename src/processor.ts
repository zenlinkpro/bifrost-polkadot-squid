...
import { CHAIN_NODE } from "./config"

const processor = new SubstrateBatchProcessor()
  .setGateway(lookupArchive('bifrost-polkadot', { release: 'ArrowSquid' }))
  .setRpcEndpoint({
    ...
    for (let event of block.events) {
      switch (event.name) {
        ...
        case 'Tokens.Deposited':
          await handleTokenDeposited({ ...ctx, block: block.header, event: event }, TOEKN_EVENT_TYPE.Tokens)
          break
        case 'Tokens.Withdrawn':
          await handleTokenWithdrawn({ ...ctx, block: block.header, event: event }, TOEKN_EVENT_TYPE.Tokens)
          break
        case 'Tokens.Transfer':
          await handleTokenTransfer({ ...ctx, block: block.header, event: event }, TOEKN_EVENT_TYPE.Tokens)
          break
        case 'ZenlinkProtocol.LiquidityAdded':
          await handleLiquidityAdded({ ...ctx, block: block.header, event: event })
          break
        case 'ZenlinkProtocol.LiquidityRemoved':
          await handleLiquidityRemoved({ ...ctx, block: block.header, event: event })
          break
        case 'ZenlinkProtocol.AssetSwap':
          await handleAssetSwap({ ...ctx, block: block.header, event: event })
          break
        case 'Tokens.BalanceSet':
          await handleTokensBalanceSet({ ...ctx, block: block.header, event: event })
          break
        // farming
        case 'Farming.FarmingPoolCreated':
          await handleFarmingPoolCreated({ ...ctx, block: block.header, event: event })
          break
        case 'Farming.FarmingPoolReset':
          await handleFarmingPoolReset({ ...ctx, block: block.header, event: event })
          break
        case 'Farming.FarmingPoolClosed':
          await handleFarmingPoolClosed({ ...ctx, block: block.header, event: event })
          break
        case 'Farming.FarmingPoolKilled':
          await handleFarmingPoolKilled({ ...ctx, block: block.header, event: event })
          break
        case 'Farming.FarmingPoolEdited':
          await handleFarmingPoolEdited({ ...ctx, block: block.header, event: event })
          break
        case 'Farming.Charged':
          await handleFarmingCharged({ ...ctx, block: block.header, event: event })
          break
        case 'Farming.Deposited':
          await handleFarmingDeposited({ ...ctx, block: block.header, event: event })
          break
        case 'Farming.Withdrawn':
          await handleFarmingWithdrawn({ ...ctx, block: block.header, event: event })
          break
        case 'Farming.Claimed':
          await handleFarmingClaimed({ ...ctx, block: block.header, event: event })
          break
        case 'Farming.WithdrawClaimed':
          await handleFarmingWithdrawClaimed({ ...ctx, block: block.header, event: event })
          break
        case 'Farming.GaugeWithdrawn':
          await handleFarmingGaugeWithdrawn({ ...ctx, block: block.header, event: event })
          break
        default:
          break
      }
